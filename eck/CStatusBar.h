...
public:
    ...
    {
        return (BOOL)SendMessageW(SB_GETBORDERS, 0, (LPARAM)pi);
    }
    ...
    {
        return (HICON)SendMessageW(SB_GETICON, idx, 0);
    }
    ...
    {
        return (int)SendMessageW(SB_GETPARTS, cParts, (LPARAM)pRights);
    }

    EckInline BOOL GetRect(int idx, _Out_ RECT* prc) const noexcept
    {
        return (BOOL)SendMessageW(SB_GETRECT, idx, (LPARAM)prc);
    }
...
#pragma warning(disable: 6054)// 可能未添加终止NULL
    ...
    {
        const auto lRet = SendMessageW(SB_GETTEXTW, idx, (LPARAM)pszBuf);
        if (pDrawFlags)
...
#pragma warning(pop)
    ...
    EckInline int GetPartTextLength(int idx,
        _Out_opt_ int* pDrawFlags = nullptr) const noexcept
    {
        const auto lRet = SendMessageW(SB_GETTEXTLENGTHW, idx, 0);
        if (pDrawFlags)
            *pDrawFlags = HIWORD(lRet);
        return LOWORD(lRet);
    }
    ...
    {
        SendMessageW(SB_GETTIPTEXTW, MAKEWPARAM(idx, cchBuf), (LPARAM)pszBuf);
    }

    EckInline BOOL IsSimple() const noexcept
    {
        return (BOOL)SendMessageW(SB_ISSIMPLE, 0, 0);
    }

    EckInline COLORREF SetBackgroundColor(COLORREF cr) const noexcept
    {
        return (COLORREF)SendMessageW(SB_SETBKCOLOR, 0, (LPARAM)cr);
    }
    ...
    {
        return (BOOL)SendMessageW(SB_SETICON, idx, (LPARAM)hIcon);
    }

    EckInline void SetMinimumHeight(int cy) const noexcept
    {
        SendMessageW(SB_SETMINHEIGHT, cy, 0);
    }
    ...
    {
        return (BOOL)SendMessageW(SB_SETPARTS, cParts, (LPARAM)pWidths);
    }

    EckInline BOOL SetPartText(int idx, _In_z_ PCWSTR pszText, int iDrawFlags = 0) const noexcept
    {
        return (BOOL)SendMessageW(SB_SETTEXTW, MAKEWORD(idx, iDrawFlags), (LPARAM)pszText);
    }

    EckInline BOOL SetPartData(int idx, LPARAM lParam) const noexcept
    {
        return (BOOL)SendMessageW(SB_SETTEXTW, MAKEWORD(idx, SBT_OWNERDRAW), lParam);
    }

    EckInline void SetTipText(int idx, _In_z_ PCWSTR pszText) const noexcept
    {
        SendMessageW(SB_SETTIPTEXTW, idx, (LPARAM)pszText);
    }

    EckInline void SetSimple(BOOL bSimple) const noexcept
    {
        SendMessageW(SB_SIMPLE, bSimple, 0);
    }
};
ECK_NAMESPACE_END