...
public:
    ...
    ECK_W_STYLE(ShowSelectionAlways, TVS_SHOWSELALWAYS);
...
public:
    ...
    {
        return (HIMAGELIST)SendMessageW(TVM_CREATEDRAGIMAGE, 0, (LPARAM)hItem);
    }

    EckInline BOOL DeleteItem(HTREEITEM hItem) const noexcept
    {
        return (BOOL)SendMessageW(TVM_DELETEITEM, 0, (LPARAM)hItem);
    }
    ...
    {
        return (HWND)SendMessageW(TVM_EDITLABELW, 0, (LPARAM)hItem);
    }

    EckInline BOOL EndEditLabel(BOOL bSave) const noexcept
    {
        return (BOOL)SendMessageW(TVM_ENDEDITLABELNOW, bSave, 0);
    }
    ...
    {
        if (bTop)
            return (BOOL)SendMessageW(TVM_SELECTITEM, TVGN_FIRSTVISIBLE, (LPARAM)hItem);
        else
            return (BOOL)SendMessageW(TVM_ENSUREVISIBLE, 0, (LPARAM)hItem);
    }
    ...
    {
        return (BOOL)SendMessageW(TVM_ENSUREVISIBLE, uOp, (LPARAM)hItem);
    }

    EckInline COLORREF GetBackgroundColor() const noexcept
    {
        return (COLORREF)SendMessageW(TVM_GETBKCOLOR, 0, 0);
    }

    EckInline int GetItemCount() const noexcept
    {
        return (int)SendMessageW(TVM_GETCOUNT, 0, 0);
    }

    EckInline HWND GetEditControl() const noexcept
    {
        return (HWND)SendMessageW(TVM_GETEDITCONTROL, 0, 0);
    }

    EckInline DWORD GetTVExtendStyle() const noexcept
    {
        return (DWORD)SendMessageW(TVM_GETEXTENDEDSTYLE, 0, 0);
    }
    ...
    {
        return (HIMAGELIST)SendMessageW(TVM_GETIMAGELIST, uType, 0);
    }
    ...
    {
        return (int)SendMessageW(TVM_GETINDENT, 0, 0);
    }

    EckInline COLORREF GetInsertMarkColor() const noexcept
    {
        return (COLORREF)SendMessageW(TVM_GETINSERTMARKCOLOR, 0, 0);
    }

    EckInline CStringW GetIncrementalSearchString() const noexcept
    {
        CStringW rs;
        int cch = (int)SendMessageW(TVM_GETISEARCHSTRINGW, 0, NULL);
        if (cch <= 0)
            return rs;
        rs.ReSize(cch);
        SendMessageW(TVM_GETISEARCHSTRINGW, 0, (LPARAM)rs.Data());
        return rs;
    }

    EckInline int GetIncrementalSearchString(_In_opt_ PWSTR pszBuf) const noexcept
    {
        return (int)SendMessageW(TVM_GETISEARCHSTRINGW, 0, (LPARAM)pszBuf);
    }

    EckInline BOOL GetItem(_Inout_ TVITEMEXW* ptvi) const noexcept
    {
        return (BOOL)SendMessageW(TVM_GETITEMW, 0, (LPARAM)ptvi);
    }

    EckInline int GetItemHeight() const noexcept
    {
        return (int)SendMessageW(TVM_GETITEMHEIGHT, 0, 0);
    }
    ...
    {
        *(HTREEITEM*)prc = hItem;
        return (BOOL)SendMessageW(TVM_GETITEMRECT, bOnlyText, (LPARAM)prc);
    }

    EckInline UINT GetItemState(HTREEITEM hItem, UINT uMask) const noexcept
    {
        return (UINT)SendMessageW(TVM_GETITEMSTATE, (WPARAM)hItem, uMask);
    }

    EckInline COLORREF GetLineColor() const noexcept
    {
        return (COLORREF)SendMessageW(TVM_GETLINECOLOR, 0, 0);
    }

    EckInline HTREEITEM GetNextItem(HTREEITEM hItem, UINT uFlag) const noexcept
    {
        return (HTREEITEM)SendMessageW(TVM_GETNEXTITEM, uFlag, (LPARAM)hItem);
    }
    ...
    {
        return (int)SendMessageW(TVM_GETSCROLLTIME, 0, 0);
    }

    EckInline COLORREF GetTextForegroundColor() const noexcept
    {
        return (COLORREF)SendMessageW(TVM_GETTEXTCOLOR, 0, 0);
    }

    EckInline HWND GetToolTip() const noexcept
    {
        return (HWND)SendMessageW(TVM_GETTOOLTIPS, 0, 0);
    }

    EckInline int GetVisibleCount() const noexcept
    {
        return (int)SendMessageW(TVM_GETVISIBLECOUNT, 0, 0);
    }
    ...
    EckInline HTREEITEM HitTest(POINT pt, _Out_opt_ UINT* puFlags = nullptr) const noexcept
    {
        ...
        SendMessageW(TVM_HITTEST, 0, (LPARAM)&tvhti);
        if (puFlags)
            *puFlags = tvhti.flags;
        return tvhti.hItem;
    }
    ...
    {
        return (HTREEITEM)SendMessageW(TVM_INSERTITEMW, 0, (LPARAM)ptvis);
    }
    ...
    {
        return (HIMAGELIST)SendMessageW(TVM_SETIMAGELIST, uType, (LPARAM)hImageList);
    }
    ...
    {
        SendMessageW(TVM_SETINDENT, iIndent, 0);
    }

    EckInline BOOL SetInsertMark(HTREEITEM hItem, BOOL bInsertAfterItem = TRUE) const noexcept
    {
        return (BOOL)SendMessageW(TVM_SETINSERTMARK, bInsertAfterItem, (LPARAM)hItem);
    }

    EckInline COLORREF SetInsertMarkColor(COLORREF cr) const noexcept
    {
        return (COLORREF)SendMessageW(TVM_SETINSERTMARKCOLOR, 0, cr);
    }

    EckInline BOOL SetItem(_Inout_ TVITEMEXW* ptvi) const noexcept
    {
        return (BOOL)SendMessageW(TVM_SETITEM, 0, (LPARAM)ptvi);
    }
    ...
    {
        return (int)SendMessageW(TVM_SETITEMHEIGHT, cy, 0);
    }

    EckInline COLORREF SetLineColor(COLORREF cr) const noexcept
    {
        return (COLORREF)SendMessageW(TVM_SETLINECOLOR, 0, cr);
    }

    EckInline int SetScrollTime(int iTime) const noexcept
    {
        return (int)SendMessageW(TVM_SETSCROLLTIME, iTime, 0);
    }

    EckInline COLORREF SetTextForegroundColor(COLORREF cr) const noexcept
    {
        return (COLORREF)SendMessageW(TVM_SETTEXTCOLOR, 0, cr);
    }

    EckInline HWND SetToolTip(HWND hToolTip) const noexcept
    {
        return (HWND)SendMessageW(TVM_SETTOOLTIPS, (WPARAM)hToolTip, 0);
    }

    EckInline void ShowTip(HTREEITEM hItem) const noexcept
    {
        SendMessageW(TVM_SHOWINFOTIP, 0, (LPARAM)hItem);
    }
    ...
    {
        return (BOOL)SendMessageW(TVM_SORTCHILDREN, bAllChildren, (LPARAM)hItem);
    }
        ...
        _In_ PFNTVCOMPARE pfnCompare, LPARAM lParam) const noexcept
    {
        ...
        tvscb.lParam = lParam;
        return (BOOL)SendMessageW(TVM_SORTCHILDRENCB, 0, (LPARAM)&tvscb);
    }
};
ECK_NAMESPACE_END