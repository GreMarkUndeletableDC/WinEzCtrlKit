...
public:
    ...
    using FSlotMethodPointer = std::conditional_t<
        IsIntercept,
        TReturn(TClass::*)(TArguments..., Slot&),
        TReturn(TClass::*)(TArguments...)
    >;
    template<class TClass>
    using FSlotMethodPointerConst = std::conditional_t<
        IsIntercept,
        TReturn(TClass::*)(TArguments..., Slot&) const,
        TReturn(TClass::*)(TArguments...) const
    >;
...
public:
    template<class F>
    EckInline HSlot Connect(F&& Fn, USHORT uId = 0, HSlot pAfter = SlotTop) noexcept
    {
        return IntConnect(std::forward<F>(Fn), uId, (NODE*)pAfter);
    }

    template<class TClass>
    EckInline HSlot Connect(TClass* pThis, FSlotMethodPointer<TClass> pfnMethod,
    ...
    template<class TClass>
    EckInline HSlot Connect(TClass* pThis, FSlotMethodPointerConst<TClass> pfnMethod,
        USHORT uId = 0, HSlot pAfter = SlotTop) noexcept
    {
        return IntConnect(
            [pThis, pfnMethod](auto&&... Args)
