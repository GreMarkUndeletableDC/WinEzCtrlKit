...
public:
    ...
    void PostDeserialize(PCVOID pData) noexcept override
    {
        ...
        if ((((CTRLDATA_WND*)pData)->uFlags & SERDF_IMAGELIST) && pBase->cbImageList)
        {
            ...
            IImageList* pIL;
            if (SUCCEEDED(ImageList_ReadEx(ILP_NORMAL, &Stream, IID_PPV_ARGS(&pIL))))
                SetImageList((HIMAGELIST)pIL);
            Stream.AssertReference(1);
        }

        auto pItem = (CTRLDATA_COMBOBOXEX::ITEM*)PointerStepBytes(pBase, sizeof(CTRLDATA_COMBOBOXEX) +
        ...
        EckCounter(-pBase->Base.cItem, i)
        {
            ...
            pItem = (CTRLDATA_COMBOBOXEX::ITEM*)PointerStepBytes(pItem, sizeof(CTRLDATA_COMBOBOXEX::ITEM) +
