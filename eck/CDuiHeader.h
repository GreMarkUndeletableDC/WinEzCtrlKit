...
private:
	...
	void DragReCalculateTargetPosition() noexcept
	{
		...
		{
			if (OrderToIndex(io) == m_idxInsertMark)
	...
	float DragCalculateInsertMarkPosition() const noexcept
	{
		...
		EckCounter(GetItemCount(), io)
		{
			...
			if (e.idxActual == m_idxInsertMark)
				break;
			x += e.cx;
		}
		return x;
	...
	LRESULT OnEvent(UINT uMsg, WPARAM wParam, LPARAM lParam) noexcept override
	{
		...
		case WM_MOUSEMOVE:
		{
			...
			else if (m_idxDrag >= 0 && m_bDraggable)
			{
				if (m_bDragging)
				{
					const auto idxIns = OrderToIndex(DragCalculateInsertOrder(ht.pt.x));
