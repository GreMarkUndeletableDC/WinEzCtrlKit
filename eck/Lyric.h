...
struct DIV
{
    std::wstring_view svDiv1{};
    std::wstring_view svDiv2{};
};
...
private:
    ...
    void MgpSplitMainAndTranslation(const std::vector<DIV_POS>& vPos) noexcept
    {
        ...
        {
            auto& Line = m_vLine[i];
            const auto& Pos = vPos[i];
            if (Pos.idxDiv < 0 || Line.cchTranslation)
                continue;
            const auto& Div = m_vDiv[Pos.idxDiv];
            if (Pos.pos2 >= 0)
            {
                Line.cchLrc = (USHORT)Pos.pos1;
                Line.pszTranslation = Line.pszLrc + Pos.pos1 + Div.svDiv1.size();
                Line.cchTranslation = USHORT(Pos.pos2 - Pos.pos1 - Div.svDiv1.size());
                DbgCutString(Line);
            }
            else
            {
                const auto cchTotal = Line.cchLrc;
                Line.cchLrc = (USHORT)Pos.pos1;
                Line.pszTranslation = Line.pszLrc + Pos.pos1 + Div.svDiv1.size();
                Line.cchTranslation = USHORT(cchTotal - Pos.pos1 - Div.svDiv1.size());
                DbgCutString(Line);
            }
        }
    }
    ...
    void MgpPostProcess() noexcept
    {
        ...
        if (!m_vDiv.empty())
        {
            std::vector<DIV_POS> vPos(m_vLine.size());
            if (MgpScanDivider(vPos))
                MgpSplitMainAndTranslation(vPos);
        }
    }
