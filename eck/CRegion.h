...
private:
    ...
    std::unique_ptr<ComplexRegion> m_pComplex{};
#ifdef _DEBUG
    ...
    constexpr static void SimplifyXPair(std::vector<XPair>& vXPair) noexcept
    {
        if (vXPair.size() <= 1)
            return;
    ...
    constexpr static void XPairIntersect(const XPair* xPair1, const XPair* xPairEnd1,
        const XPair* xPair2, const XPair* xPairEnd2, std::vector<XPair>& vResult) noexcept
    {
        vResult.clear();
        do
        {
...
public:
    ...
    public:
        ...
        constexpr void EndLine() noexcept
        {
        ...
        constexpr void AddPoint(int x) noexcept
        {
            EckAssert(m_y != InfMin && m_pComplex && m_pCurrYCat);
    ...
    CRegion() = default;
    CRegion(const CRegion& x) noexcept : m_rc{ x.m_rc }
    {
        EckAssert(!x.m_bRecordingPoint);
        if (x.m_pComplex)
            m_pComplex = std::make_unique<ComplexRegion>(*x.m_pComplex);
    }
    CRegion(CRegion&& x) noexcept :m_rc{ x.m_rc }, m_pComplex{ std::move(x.m_pComplex) }
    {
        EckAssert(!x.m_bRecordingPoint);
    ...
    CRegion& operator=(const CRegion& x) noexcept
    {
    ...
    CRegion& operator=(CRegion&& x) noexcept
    {
        EckAssert(!x.m_bRecordingPoint && !m_bRecordingPoint);
    ...
    void SetRect(const RECT* prc, int cRc) noexcept
    {
    ...
    BOOL SetHRgn(HRGN hRgn) noexcept
    {
        EckAssert(!m_bRecordingPoint);
    ...
    GpStatus SetGpRegion(GpRegion* pRegion) noexcept
    {
        EckAssert(!m_bRecordingPoint);
        GpStatus gps;
        GpMatrix* pMat;
    ...
    constexpr void GetRect(RECT* prc, size_t cMax) const noexcept
    {
        if (!cMax)
            return;
        if (m_pComplex)
    ...
    EckInline constexpr void GetBound(RECT& rcBound) const noexcept
    {
        rcBound = m_rc;
    }

    [[nodiscard]] CRegion Intersect(const CRegion& Rgn) const noexcept
    {
        EckAssert(!m_bRecordingPoint);
        CRegion RgnResult{};
    ...
    [[nodiscard]] CRegion Union(const CRegion& Rgn) const noexcept
    {
        EckAssert(!m_bRecordingPoint);
        CRegion RgnResult{};
    ...
    [[nodiscard]] CRegion Difference(const CRegion& Rgn) const noexcept
    {
        EckAssert(!m_bRecordingPoint);
        CRegion RgnResult{};
    ...
    [[nodiscard]] CRegion SymmetricDifference(const CRegion& Rgn) const noexcept
    {
        EckAssert(!m_bRecordingPoint);
        CRegion RgnResult{};
    ...
    [[nodiscard]] CPtRecorder RpBegin() noexcept
    {
#ifdef _DEBUG
        ...
        if (!m_pComplex)
            ...
            m_pComplex->Reset();
        return CPtRecorder(m_pComplex.get(), this);
    }
...
EckInline [[nodiscard]] CRegion operator|(const CRegion& Rgn1, const CRegion& Rgn2) noexcept
{
