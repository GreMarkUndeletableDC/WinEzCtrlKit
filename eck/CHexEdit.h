...
public:
    ...
    ECK_W_NONATTACHABLE_NO_CONS(CHexEdit);
    ECK_W_CREATE_CLASS_INST(WCN_HEXEDIT, g_hInstance);
private:
    struct CHAR_COL
    {
        UINT uCp;
    ...
    void ReCalculateFirstVisible(int posSb) noexcept
    {
        m_posFirstVisible = posSb * D.cCol;
    }

    void UpdateSystemConfig() noexcept
    {
    ...
    void UpdateDCAttributes() noexcept
    {
    ...
    void UpdateCaretPosition() noexcept
    {
        ...
        {
            SetCaretPos(
                -ScbGetPosition(SB_HORZ) + m_cxAddress + GetColumnWidth() * idxH +
        ...
        {
            SetCaretPos(
                -ScbGetPosition(SB_HORZ) + m_cxAddress + m_cxData + m_cxChar * idxH +
...
public:
    ...
    EckInlineNdCe int GetVisibleRowCount() const noexcept
    {
    ...
    EckInlineNdCe int GetColumnCount() const noexcept
    {
        return D.cCol;
    }

    EckInlineNdCe int GetColumnPerGroup() const noexcept
    {
        return D.cColPerGroup;
    }

    EckInlineNdCe int GetColumnWidth() const noexcept
    {
        return m_cxChar * 2 + D.cxGap;
    }

    EckInlineNdCe int GetCharColumnWidth() const noexcept
    {
        return (m_cxChar + 1) * D.cCol + D.cxGap;
    }

    EckInlineNdCe int GetHeaderHeight() const noexcept
    {
        return m_cyChar + D.cyGap;
    }

    EckInlineNdCe int GetRowHeight() const noexcept
    {
        return m_cyChar + D.cyGap;
    }

    EckInlineNdCe int GetFirstVisibleRow() const noexcept
    {
        return int(m_posFirstVisible / D.cCol);
    }

    int InsertCharColumn(UINT uCodePage, int idx = -1) noexcept
    {
    ...
    size_t HitTest(_Inout_ HEHITTEST& heht) const noexcept
    {
        const int xStart = -ScbGetPosition(SB_HORZ);
        heht.uFlags = 0u;
