...
public:
    ...
    {
        return (int)SendMessageW(PBM_DELTAPOS, iDelta, 0);
    }

    EckInline COLORREF GetBarColor() const noexcept
    {
        return (COLORREF)SendMessageW(PBM_GETBARCOLOR, 0, 0);
    }

    EckInline COLORREF GetBackgroundColor() const noexcept
    {
        return (COLORREF)SendMessageW(PBM_GETBKCOLOR, 0, 0);
    }

    EckInline int GetPosition() const noexcept
    {
        return (int)SendMessageW(PBM_GETPOS, 0, 0);
    }

    EckInline PBRANGE GetRange() const noexcept
    {
        PBRANGE r;
        SendMessageW(PBM_GETRANGE, 0, (LPARAM)&r);
        return r;
    }

    EckInline int GetMinimum() const noexcept
    {
        return (int)SendMessageW(PBM_GETRANGE, TRUE, 0);
    }

    EckInline int GetMaximum() const noexcept
    {
        return (int)SendMessageW(PBM_GETRANGE, FALSE, 0);
    }
    ...
    {
        return (int)SendMessageW(PBM_SETSTATE, iState, 0);
    }

    EckInline int SetStep(int i) const noexcept
    {
        return (int)SendMessageW(PBM_SETSTEP, i, 0);
    }

    EckInline int StepIt() const noexcept
    {
        return (int)SendMessageW(PBM_STEPIT, 0, 0);
    }
};
ECK_NAMESPACE_END