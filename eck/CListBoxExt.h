...
#pragma warning(pop)
    ...
    int HitTestCheckBox(POINT pt, int* pidxItem = nullptr) noexcept
    {
        POINT ptScr = pt;
    ...
    */
public:
    CListBoxExt() noexcept
    {
        m_InfoEx.idxCurrSel = -1;
        m_InfoEx.iAlignV = 1;

        SetSelectionColor(0, CLR_DEFAULT);
        SetSelectionColor(1, CLR_DEFAULT);
        SetColor(0, CLR_DEFAULT);
        SetColor(1, CLR_DEFAULT);
    }
    ...
    void SetItem(int idx, PCWSTR pszString, PCWSTR pszTip, const LBITEMCOMMINFO& CommInfo) noexcept
    {
    ...
    BOOL DeleteString(int iPos) noexcept
    {
        if (iPos < 0)
        {
    ...
    EckInline void SwapItem(int idx1, int idx2) noexcept
    {
        std::swap(m_ItemsInfo[idx1], m_ItemsInfo[idx2]);
        Redraw();
    }

    EckInline void MoveItem(int idxSrc, int idxDst) noexcept
    {
    ...
    void SetColor(int idx, COLORREF cr) noexcept
    {
        if (idx)
        {
            m_InfoEx.crBK = cr;
    ...
    EckInline int GetColor(int idx) noexcept
    {
        if (idx)
    ...
    EckInline void SetAutoSort(BOOL bAutoSort) noexcept
    {
    ...
    EckInline void SetToolTip(BOOL bToolTip) noexcept
    {
        m_InfoEx.bToolTip = bToolTip;
    }

    EckInline BOOL GetToolTip() noexcept
    {
        return m_InfoEx.bToolTip;
    }

    EckInline void SetDirectory(PCWSTR pszDir) noexcept
    {
        m_rsDir = pszDir;
        AddFile();
    }
    ...
    EckInline void SetFilePattern(PCWSTR pszFilePattern) noexcept
    {
    ...
    void SetFileAttribute(UINT uAttr) noexcept
    {
        m_InfoEx.uFileAttr = uAttr;
        AddFile();
    }

    UINT GetFileAttribute() noexcept
    {
        return m_InfoEx.uFileAttr;
    }

    void SetFileTime(BOOL bMaxTime, DATE date) noexcept
    {
        SYSTEMTIME st;
        FILETIME ft;
    ...
    DATE GetFileTime(BOOL bMaxTime) noexcept
    {
        SYSTEMTIME st;
        FILETIME ft;
        DATE date;
    ...
    HIMAGELIST SetImageList(HIMAGELIST hImageList) noexcept
    {
    ...
    EckInline HIMAGELIST GetImageList() noexcept
    {
        return m_hImageList;
    }

    void SetSelectionColor(int idx, COLORREF cr) noexcept
    {
        if (idx)
        {
            m_InfoEx.crSelBK = cr;
    ...
    EckInline COLORREF GetSelectionColor(int idx) noexcept
    {
        if (idx)
    ...
    EckInline void SetEllipsis(BOOL bEllipsis) noexcept
    {
        m_InfoEx.bEllipsis = bEllipsis;
    }

    EckInline BOOL GetEllipsis() noexcept
    {
        return m_InfoEx.bEllipsis;
    }

    EckInline void SetAlignment(BOOL bHAlign, int iAlign) noexcept
    {
        if (bHAlign)
    ...
    EckInline int GetAlignment(BOOL bHAlign) noexcept
    {
        if (bHAlign)
    ...
    EckInline void SetCheckBoxMode(int iCheckBoxMode) noexcept
    {
        m_InfoEx.iCheckBoxMode = iCheckBoxMode;
        Redraw();
    }

    EckInline int GetCheckBoxMode() noexcept
    {
        return m_InfoEx.iCheckBoxMode;
    }

    EckInline void SetBalloonToolTip(BOOL bBalloonToolTip) noexcept
    {
        m_InfoEx.bBalloonToolTip = bBalloonToolTip;
        ModifyWindowStyle(m_hToolTip, bBalloonToolTip ? TTS_BALLOON : 0, TTS_BALLOON);
    }

    EckInline BOOL GetBalloonToolTip() noexcept
    {
