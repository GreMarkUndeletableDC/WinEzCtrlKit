        ...
        EckCounter(cy, j)
        {
            EckCounter(cx, i)
            {
                *Tex.CpuData<UINT>(x + i, y + j) =
                    UINT(0x00FF'FFFF | (m_Ect[m_vTemp[j * cx + i]] << 24));
            }
        }
        return S_OK;
    }
        ...
        _Out_ TId& id) noexcept
    {
        ...
        EckCounter(cy, j)
        {
            EckCounter(cx, i)
            {
                ...
                *Tex.CpuData<UINT>(x + i, y + j) =
                    UINT(0x00FF'FFFF | (m_Ect[byGray] << 24));
            }
        }
        return S_OK;
    }
...
public:
    CFontAtlasManager() noexcept
    {
        InitializeEnhancedContrastTable(0.5f);
    }
    ...
    void InitializeEnhancedContrastTable(float k) noexcept
    {
        constexpr BYTE MaxAlpha = 255;
        for (BYTE i = 1; i < MaxAlpha; ++i)
        {
            const auto a = i * (1.f / MaxAlpha);
            const auto t = (a * (k + 1.f)) / (a * k + 1.f);
            m_Ect[i] = BYTE(t * MaxAlpha + 0.5f);
        }
        m_Ect[0] = 0;
        m_Ect[MaxAlpha] = MaxAlpha;
    }
