...
struct CoroPromiseBase
{
    ...
    auto final_suspend() noexcept
    {
        struct Awaiter
        {
            ...
            bool await_suspend(std::coroutine_handle<>) const noexcept
            {
                ...
                // 状态字段设置为Completed
                auto e{ State::Initial };
                Pro.m_eState.compare_exchange_strong(
                    e, State::Completed, std::memory_order_acq_rel);
                Pro.m_Lk.EnterRead();
                const auto hNext{ Pro.m_hCoroNext };
                Pro.m_Lk.LeaveRead();
                if (hNext)// 恢复到等待方
                    hNext.resume();
    ...
    bool SetNextCoroutine(std::coroutine_handle<> hCoroNext_) noexcept
    {
        ...
        // 原状态不可能为HasNext
        auto e{ State::Initial };
        return m_eState.compare_exchange_strong(
            e, State::HasNext, std::memory_order_release);
    }

    EckInline BOOL IsCompleted() const noexcept
    {
        return m_eState.load(std::memory_order_relaxed) == State::Completed;
    }

    BOOL TryCancel() noexcept
    {
        auto e{ State::Initial };
        m_eState.compare_exchange_strong(e, State::Cancelled, std::memory_order_acq_rel);
        EckAssert(e != State::Cancelled);// 已被取消过
        if (e == State::Initial)
            goto Success;
        e = State::HasNext;
        m_eState.compare_exchange_strong(e, State::Cancelled, std::memory_order_acq_rel);
    ...
    {
        return m_eState.load(std::memory_order_relaxed) == State::Cancelled;
    }
...
EckInline auto CoroCaptureUiThread(ThreadContext* ptc = nullptr)
{
    struct Context
    {
    private:
