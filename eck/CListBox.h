...
public:
    ...
    ECK_W_STYLE(ComboList, LBS_COMBOBOX);
    ECK_W_STYLE(DisableNoScroll, LBS_DISABLENOSCROLL);
    ECK_W_STYLE(ExtendedSelect, LBS_EXTENDEDSEL);
    ECK_W_STYLE(HasStrings, LBS_HASSTRINGS);
    ECK_W_STYLE(MultiColumn, LBS_MULTICOLUMN);
    ECK_W_STYLE(MultipleSelect, LBS_MULTIPLESEL);
    ECK_W_STYLE(NoData, LBS_NODATA);
    ECK_W_STYLE(NoIntegralHeight, LBS_NOINTEGRALHEIGHT);
    ECK_W_STYLE(NoRedraw, LBS_NOREDRAW);
    ECK_W_STYLE(NoSelection, LBS_NOSEL);
    ECK_W_STYLE(Notify, LBS_NOTIFY);
    ECK_W_STYLE(OwnerDrawFixed, LBS_OWNERDRAWFIXED);
    ECK_W_STYLE(OwnerDrawVariable, LBS_OWNERDRAWVARIABLE);
    ECK_W_STYLE(Sort, LBS_SORT);
    ECK_W_STYLE(Standard, LBS_STANDARD);
    ECK_W_STYLE(UseTabStops, LBS_USETABSTOPS);
    ECK_W_STYLE(WantKeyboradInput, LBS_WANTKEYBOARDINPUT);

    EckInline int AddString(PCWSTR psz) const noexcept
    {
        return (int)SendMessageW(LB_ADDSTRING, 0, (LPARAM)psz);
    }

    EckInline int AddString(LPARAM lParam) const noexcept
    {
        return (int)SendMessageW(LB_ADDSTRING, 0, lParam);
    }
    ...
    {
        return (int)SendMessageW(LB_DELETESTRING, idx, 0);
    }
    ...
    {
        return (int)SendMessageW(LB_DIR, uFlags, (LPARAM)pszPath);
    }
    ...
    {
        return (int)SendMessageW(LB_FINDSTRING, idxStart, (LPARAM)pszText);
    }
    ...
    {
        return (int)SendMessageW(LB_FINDSTRINGEXACT, idxStart, (LPARAM)pszText);
    }

    EckInline int GetAnchorIndex() const noexcept
    {
        return (int)SendMessageW(LB_GETANCHORINDEX, 0, 0);
    }
    ...
    {
        return (int)SendMessageW(LB_GETCARETINDEX, 0, 0);
    }

    EckInline int GetItemCount() const noexcept
    {
        return (int)SendMessageW(LB_GETCOUNT, 0, 0);
    }
    ...
    {
        return (int)SendMessageW(LB_GETHORIZONTALEXTENT, 0, 0);
    }

    EckInline LPARAM GetItemData(int idx) const noexcept
    {
        return SendMessageW(LB_GETITEMDATA, idx, 0);
    }

    EckInline int GetItemHeight(int idx) const noexcept
    {
        return (int)SendMessageW(LB_GETITEMHEIGHT, idx, 0);
    }

    EckInline BOOL GetItemRect(int idx, _Out_ RECT* prc) const noexcept
    {
        return (SendMessageW(LB_GETITEMRECT, idx, (LPARAM)prc) != LB_ERR);
    }

    EckInline int GetItemCountPreColumn() const noexcept
    {
        return (int)SendMessageW(LB_GETLISTBOXINFO, 0, 0);
    }

    EckInline LCID GetLocale() const noexcept
    {
        return (LCID)SendMessageW(LB_GETLOCALE, 0, 0);
    }

    EckInline int GetSelection(int idx) const noexcept
    {
        return (SendMessageW(LB_GETSEL, idx, 0) > 0);
    }
    ...
    {
        return (int)SendMessageW(LB_GETSELCOUNT, 0, 0);
    }
    ...
    {
        return (int)SendMessageW(LB_GETSELITEMS, c, (LPARAM)piSelItems);
    }

    BOOL GetItemText(int idx, Eck_Append_buffer_ CStringW& rs) const noexcept
    {
        auto cch = GetItemTextLength(idx);
        if (cch < 0)
            return FALSE;
        if (!cch)
            return TRUE;
    ...
    {
        return (int)SendMessageW(LB_GETTEXT, idx, (LPARAM)pszBuf);
    }
    ...
    {
        return (int)SendMessageW(LB_GETTEXTLEN, idx, 0);
    }

    EckInline int GetTopIndex() const noexcept
    {
        return (int)SendMessageW(LB_GETTOPINDEX, 0, 0);
    }
    ...
    {
        return (int)SendMessageW(LB_INSERTSTRING, idxPos, (LPARAM)psz);
    }

    EckInline int InsertString(LPARAM lParam, int idxPos = -1) const noexcept
    {
        return (int)SendMessageW(LB_INSERTSTRING, idxPos, lParam);
    }
    ...
    {
        SendMessageW(LB_RESETCONTENT, 0, 0);
    }
    ...
    {
        return (int)SendMessageW(LB_SELECTSTRING, idxStart, (LPARAM)pszText);
    }
    ...
    EckInline BOOL SelectItemRange(int idxStart, int idxEnd, BOOL bSel) const noexcept
    {
        ...
        return (SendMessageW(LB_SELITEMRANGEEX, idxStart, idxEnd) != LB_ERR);
    }

    EckInline BOOL SetAnchorIndex(int idx) const noexcept
    {
        return (SendMessageW(LB_SETANCHORINDEX, idx, 0) != LB_ERR);
    }

    EckInline BOOL SetCaretIndex(int idx, BOOL bNoFullVisible = FALSE) const noexcept
    {
        return (SendMessageW(LB_SETCARETINDEX, idx, bNoFullVisible) != LB_ERR);
    }

    EckInline void SetColumnWidth(int cxColumn) const noexcept
    {
        SendMessageW(LB_SETCOLUMNWIDTH, cxColumn, 0);
    }

    EckInline int SetCount(int cItems) const noexcept
    {
        return (int)SendMessageW(LB_SETCOUNT, cItems, 0);
    }

    EckInline BOOL SetCurrentSelection(int idxSel = -1) const noexcept
    {
        int iRet = (int)SendMessageW(LB_SETCURSEL, idxSel, 0);
        if (idxSel < 0)
            return TRUE;
        else
            return (iRet != LB_ERR);
    ...
    {
        SendMessageW(LB_SETHORIZONTALEXTENT, iHorizontalExtent, 0);
    }

    EckInline BOOL SetItemData(int idx, LPARAM lParam) const noexcept
    {
        return (SendMessageW(LB_SETITEMDATA, idx, lParam) != LB_ERR);
    }
    ...
    {
        return (SendMessageW(LB_SETITEMHEIGHT, idx, cy) != LB_ERR);
    }
    ...
    {
        return (LCID)SendMessageW(LB_SETLOCALE, lcid, 0);
    }
    ...
    {
        return (SendMessageW(LB_SETSEL, bSel, idx) != LB_ERR);
    }

    EckInline BOOL SetTabStop(_In_reads_(c) const int* piTabStop, int c) const noexcept
    {
        return (BOOL)SendMessageW(LB_SETTABSTOPS, c, (LPARAM)piTabStop);
    }

    EckInline BOOL SetTopIndex(int idx) const noexcept
    {
        return (SendMessageW(LB_SETTOPINDEX, idx, 0) != LB_ERR);
    }
