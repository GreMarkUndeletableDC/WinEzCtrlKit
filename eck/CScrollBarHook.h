...
private:
    ...
    GenH:;
        ...
        return;
    GenV:;
        m_pWnd->SendMessageW(WM_VSCROLL, MAKEWPARAM(uCode, 0), 0);
    }
        ...
        WPARAM wParam, LPARAM lParam, Slot& Ctx) noexcept
    {
        ...
        case WM_MOUSEMOVE:
        {
            if (m_eLBtnDownPart == Part::ThumbV)
            {
                ...
                {
                    m_pWnd->SendMessageW(WM_VSCROLL, MAKEWPARAM(SB_THUMBTRACK, m_ViewTracking.GetPosition()), 0);
            ...
            else if (m_eLBtnDownPart == Part::ThumbH)
            {
                ...
                if (m_ViewTracking.GetPosition() != m_nLastTrackPos)
                {
                    m_pWnd->SendMessageW(WM_HSCROLL, MAKEWPARAM(SB_THUMBTRACK, m_ViewTracking.GetPosition()), 0);
                    m_nLastTrackPos = m_ViewTracking.GetPosition();
                }
                return 0;
            }
        ...
        case WM_LBUTTONUP:
        {
            if (m_eLBtnDownPart != Part::Invalid)
            {
                ...
                if (m_eLBtnDownPart == Part::ThumbV)
                {
                    ...
                    m_pWnd->SendMessageW(WM_VSCROLL, MAKEWPARAM(SB_THUMBPOSITION, m_ViewTracking.GetPosition()), 0);
                    m_pWnd->SendMessageW(WM_VSCROLL, MAKEWPARAM(SB_ENDSCROLL, 0), 0);
                }
                else if (m_eLBtnDownPart == Part::ThumbH)
                {
                    m_ViewTracking.OnLButtonUp();
                    m_pWnd->SendMessageW(WM_HSCROLL, MAKEWPARAM(SB_THUMBPOSITION, m_ViewTracking.GetPosition()), 0);
                    m_pWnd->SendMessageW(WM_HSCROLL, MAKEWPARAM(SB_ENDSCROLL, 0), 0);
                }
