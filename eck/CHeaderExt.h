...
private:
    ...
    void PrepareTextColor(HDC hDC, const ITEM& e) noexcept
    {
        ...
        if (e.crText != CLR_DEFAULT)
            ...
            SetTextColor(hDC, m_ptc->crDefText);
    }

    void PrepareTextColorSubText(HDC hDC, const ITEM& e) noexcept
    {
        ...
        if (e.crTextSubText != CLR_DEFAULT)
            ...
            SetTextColor(hDC, m_ptc->crTip1);
    }

    void PrepareTextColorFilter(HDC hDC, const ITEM& e) noexcept
    {
    ...
    LRESULT OnItemPrePaint(NMCUSTOMDRAW* pnmcd) noexcept
    {
    ...
    void InternalAttachNew(HWND hWnd) noexcept
    {
        m_iDpi = GetDpi(hWnd);
...
public:
    ...
    LRESULT OnMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) noexcept override
    {
        ...
        case HDM_HITTEST:
        {
            ...
            if (lResult >= 0)
            {
                ...
                // NOTE 标准控件Bug：自定义绘制时懒加载数据未正确计算，导致拆分按钮命中测试不正常
        ...
        WPARAM wParam, LPARAM lParam, BOOL& bProcessed) noexcept override
    {
        ...
        return CHeader::OnNotifyMessage(hParent, uMsg, wParam, lParam, bProcessed);
    }

    EckInlineCe void HeSetBkColor(COLORREF cr) noexcept { m_crBk = cr; }
    EckInlineNdCe COLORREF HeGetBkColor() const noexcept { return m_crBk; }

    EckInlineCe void HeSetTextBkColor(COLORREF cr) noexcept { m_crTextBk = cr; }
    EckInlineNdCe COLORREF HeGetTextBkColor() const noexcept { return m_crTextBk; }

    EckInlineCe void HeSetTextColor(COLORREF cr) noexcept { m_crText = cr; }
    EckInlineNdCe COLORREF HeGetTextColor() const noexcept { return m_crText; }

    EckInlineCe void HeSetSubTextColor(COLORREF cr) noexcept { m_crTextSubText = cr; }
    EckInlineNdCe COLORREF HeGetSubTextColor() const noexcept { return m_crTextSubText; }

    EckInlineCe void HeSetSubTextBkColor(COLORREF cr) noexcept { m_crTextBkSubText = cr; }
    EckInlineNdCe COLORREF HeGetSubTextBkColor() const noexcept { return m_crTextBkSubText; }

    EckInlineCe void HeSetFilterTextColor(COLORREF cr) noexcept { m_crTextFilter = cr; }
    EckInlineNdCe COLORREF HeGetFilterTextColor() const noexcept { return m_crTextFilter; }

    EckInlineCe void HeSetFilterTextBkColor(COLORREF cr) noexcept { m_crTextBkFilter = cr; }
    EckInlineNdCe COLORREF HeGetFilterTextBkColor() const noexcept { return m_crTextBkFilter; }

    EckInlineNdCe auto& HeGetItemData(int idx) noexcept { return m_vData[idx]; }
    EckInlineNdCe auto& HeGetItemData(int idx) const noexcept { return m_vData[idx]; }

    void HeSetMainTextFont(HFONT hFont) noexcept
    {
    ...
    EckInlineNdCe HFONT HeGetMainTextFont() const noexcept { return m_hFontMainText; }

    EckInlineCe void HeSetAutoDarkMode(BOOL b) noexcept { m_bAutoDarkMode = b; }
    EckInlineNdCe BOOL HeGetAutoDarkMode() const noexcept { return m_bAutoDarkMode; }

    EckInlineCe void HeSetUseExtText(BOOL b) noexcept { m_bUseExtText = b; }
    EckInlineNdCe BOOL HeGetUseExtText() const noexcept { return m_bUseExtText; }

    EckInlineCe void HeSetRepairDoubleClick(BOOL b) noexcept { m_bRepairDbClick = b; }
    EckInlineNdCe BOOL HeGetRepairDoubleClick() const noexcept { return m_bRepairDbClick; }

    EckInlineCe void HeSetShowEmptyMainExtText(BOOL b) noexcept { m_bShowEmptyMainExtText = b; }
    EckInlineNdCe BOOL HeGetShowEmptyMainExtText() const noexcept { return m_bShowEmptyMainExtText; }
