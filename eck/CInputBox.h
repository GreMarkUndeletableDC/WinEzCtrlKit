...
private:
    ...
    void UpdateTextMetrics() noexcept
    {
    ...
    void UpdateDpi(int iDpi) noexcept
    {
        m_iDpi = iDpi;
        UpdateDpiSize(m_Ds, iDpi);
    }

    void UpdateDpiControlSize() noexcept
    {
...
public:
    ...
    BOOL OnInitializeDialog(HWND hFocus, LPARAM lParam) noexcept override
    {
        UpdateDpi(GetDpi(Handle));
        ...
        if (m_pOpt->uFlags & IPBF_FIXWIDTH)
        {
            ...
            if (IsBitSet(m_pOpt->uFlags, IPBF_CENTERPARENT))
                pt = CalculateCenterWindowPosition(HWND(lParam), rc.right - rc.left, rc.bottom - rc.top);
            else if (IsBitSet(m_pOpt->uFlags, IPBF_CENTERSCREEN))
                pt = CalculateCenterWindowPosition(nullptr, rc.right - rc.left, rc.bottom - rc.top);
            else
                pt = { m_pOpt->x,m_pOpt->y };
            SetWindowPos(Handle, nullptr, pt.x, pt.y, rc.right - rc.left, rc.bottom - rc.top,
                SWP_NOZORDER | SWP_NOACTIVATE);
        }
        m_ED.SelectAll();
    ...
    INT_PTR CreateModalDialog(HWND hParent, void* pData = nullptr) noexcept override
    {
        if (!pData)
        {
            EckDbgPrint(L"** WARNING ** CInputBox::CreateModalDialog: pData is nullptr.");
            EckDbgBreak();
            return FALSE;
        }
        ...
        InternalCreateModalDialog(0, WCN_DIALOG, m_pOpt->pszTitle,
    ...
    CEditExt::InputMode eInputMode = CEditExt::InputMode::Normal) noexcept
{
    ...
    CInputBox ib{};
    if (ib.CreateModalDialog(hParent, &Opt))
    {
