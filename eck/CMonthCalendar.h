...
public:
    ...
    ECK_W_STYLE(DayState, MCS_DAYSTATE);
    ECK_W_STYLE(MultiSelect, MCS_MULTISELECT);
    ECK_W_STYLE(WeekNumbers, MCS_WEEKNUMBERS);
    ECK_W_STYLE(NoTodayCircle, MCS_NOTODAYCIRCLE);
    ECK_W_STYLE(NoToday, MCS_NOTODAY);
    ECK_W_STYLE(NoTrailingDates, MCS_NOTRAILINGDATES);
    ECK_W_STYLE(ShortDaysOfWeek, MCS_SHORTDAYSOFWEEK);
    ECK_W_STYLE(NoSelectionChangeOnNavigate, MCS_NOSELCHANGEONNAV);
    ...
    {
        return (int)SendMessageW(MCM_GETCALENDARBORDER, 0, 0);
    }

    EckInline int GetCalendarCount() const noexcept
    {
        return (int)SendMessageW(MCM_GETCALENDARCOUNT, 0, 0);
    }

    EckInline BOOL GetGridInfomation(_Inout_ MCGRIDINFO* pmcgi) const noexcept
    {
        return (BOOL)SendMessageW(MCM_GETCALENDARGRIDINFO, 0, (LPARAM)pmcgi);
    }
    ...
    {
        return (int)SendMessageW(MCM_GETCALID, 0, 0);
    }
    ...
    {
        return (COLORREF)SendMessageW(MCM_GETCOLOR, iType, 0);
    }
    ...
    {
        return (int)SendMessageW(MCM_GETCURRENTVIEW, 0, 0);
    }

    EckInline BOOL GetCurrentSelection(_Out_ SYSTEMTIME* pst) const noexcept
    {
        return (BOOL)SendMessageW(MCM_GETCURSEL, 0, (LPARAM)pst);
    }

    EckInline UINT GetFirstDayOfWeek() const noexcept
    {
        return (UINT)SendMessageW(MCM_GETFIRSTDAYOFWEEK, 0, 0);
    }
    ...
    {
        return (int)SendMessageW(MCM_GETMAXSELCOUNT, 0, 0);
    }

    EckInline int GetMaxTodayWidth() const noexcept
    {
        return (int)SendMessageW(MCM_GETMAXTODAYWIDTH, 0, 0);
    }

    EckInline BOOL GetMinimumRect(_Out_ RECT* prc) const noexcept
    {
        return (BOOL)SendMessageW(MCM_GETMINREQRECT, 0, (LPARAM)prc);
    }

    EckInline int GetMonthDelta() const noexcept
    {
        return (int)SendMessageW(MCM_GETMONTHDELTA, 0, 0);
    }
    ...
    {
        return (int)SendMessageW(MCM_GETMONTHRANGE, iOption, (LPARAM)pst);
    }
    ...
    {
        return (COLORREF)SendMessageW(MCM_SETCOLOR, iType, (LPARAM)crColor);
    }
