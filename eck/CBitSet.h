...
public:
    ...
    template<size_t NPosition, size_t NBits>
        requires requires { NPosition < N && NPosition + NBits <= N; }
    [[nodiscard]] CBitSet<NBits> SubSet() noexcept
    {
        ...
        {
            CBitSet t{};
            CopyBits(t.Data(), NPosition, NBits);
            memcpy(x.Data(), t.Data(), sizeof(CBitSet<NBits>::m_Bits));
        }
        else
            CopyBits((TWord*)x.Data(), NPosition, NBits);
        x.Trim();
        return x;
    }
